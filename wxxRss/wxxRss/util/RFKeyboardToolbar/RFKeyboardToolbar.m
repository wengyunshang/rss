...
- (UIView*)inputAccessoryView {
    _toolbarView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, 41)];
    _toolbarView.backgroundColor = [UIColor clearColor];
    _toolbarView.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
//    [_toolbarView blurScreen:YES];
...
- (UIScrollView*)fakeToolbar {
    ...
    for (RFToolbarButton *eachButton in _buttonsToAdd) {
        originFrame = CGRectMake(UIBounds.size.width-eachButton.frame.size.width, 0, eachButton.frame.size.width, eachButton.frame.size.height);
